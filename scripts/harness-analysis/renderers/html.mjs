...
<dialog id="manual-copy-dialog" class="manual-copy-dialog" aria-labelledby="manual-copy-title" aria-describedby="manual-copy-description">
  <div class="dialog-heading">
    <div>
      <span class="label">${copy(reportData.language, "Manual fallback", "手动复制")}</span>
      <h3 id="manual-copy-title">${copy(reportData.language, "Copy the AI Fix prompt", "复制 AI 修复提示词")}</h3>
    </div>
  </div>
  <p id="manual-copy-description">${copy(reportData.language, "Automatic copy was blocked. The exact prompt is selected below.", "自动复制被阻止，下面已选中完整提示词。")}</p>
  <textarea id="manual-copy-text" readonly aria-label="${copy(reportData.language, "AI Fix prompt", "AI 修复提示词")}"></textarea>
  <div class="dialog-actions">
    <button type="button" class="action-button ghost" data-close-finding-dialog="manual-copy-dialog">${copy(reportData.language, "Close", "关闭")}</button>
  </div>
</dialog>
