...
- AC-5: Plural-sensitive copy uses i18next plural keys; `zh-CN` never needs
  plural forms while `en` keeps `_one`/`_other`.
...
- Namespaces mirror Studio domains:
  ...
  `code`, `artifacts`, `git`, `inputs`, `inspector`, `customize`, `compare`.
...
- Plural copy uses `{{count}}` with `_one`/`_other` on `en`; `zh-CN`
  provides one key.
...
5. Parallel workstreams (disjoint namespaces and files):
   - artifacts: `ArtifactsWorkspace`, `artifacts/*` (incl. pdf/docx/pptx/
     xlsx viewers, registry, preview host, diagnostics, zoom controls).
...
- Unit: i18n resource parity test; shell-model helpers under English `t`.
- Browser: language switch spec (zh-CN renders, persists, reloads);
  existing specs unchanged in English.
- Build: `harness-studio:build` succeeds; Playwright suite green; console
  and page-error free at 390px and wide viewports; screenshot of each
  changed surface for layout review.