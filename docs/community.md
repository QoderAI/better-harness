...
| Host shell and packaging | Thin, or generated only after a split trigger | `.claude-plugin/`, `.qoder-plugin/`, `.cursor-plugin/`, `.codex-plugin/`, future lifecycle shells | Install/discovery metadata and pointers to canonical owners | Qoder package distribution includes only `.qoder-plugin/`; other host shells are source-local until a dedicated package exists | `scripts/npm-package/` verification, or split adapter note plus target builder |
