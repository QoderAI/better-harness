...
| Host shell | Thin host metadata (`.claude-plugin/`, `.qoder-plugin/`, `.cursor-plugin/`, `.codex-plugin/`, or a future lifecycle shell) that exposes canonical behavior without owning product logic; the Qoder package ships only `.qoder-plugin/`. | [ARCHITECTURE.md](ARCHITECTURE.md) |
