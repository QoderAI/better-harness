...
function parseTimelineItem(value: unknown): SavedRunTimelineItem {
  ...
  if (item.kind === "message") {
    return { kind: "message", id: item.id, text: typeof item.text === "string" ? item.text : "", complete: item.complete === true, ...((item.role === "thought" || item.role === "user") ? { role: item.role } : {}), ...(Array.isArray(item.content) ? { content: item.content } : {}) };
  }
