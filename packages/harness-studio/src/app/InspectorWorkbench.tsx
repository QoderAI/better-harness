...
export function InspectorWorkbench(props: { fallback: ReactNode; reportUrl?: string; dateRange?: StudioDateRange }): React.JSX.Element {
  ...
  }, [props.reportUrl, props.dateRange, retry]);

  if (failure) return <div className="inspector-fallback-shell"><p className="inspector-fallback-message" role="status">{failure}</p><button type="button" onClick={() => setRetry((value) => value + 1)}>{inspectorT("retry")}</button>{props.fallback}</div>;
  return <div className="inspector-native-shell" aria-busy={!loaded}>
    {!loaded && <p className="inspector-native-status" role="status">{inspectorT("loading")}</p>}
    <div className="inspector-native-host" aria-label={inspectorT("workbenchAria")} ref={(host) => {
      if (host) {
        const nextShadow = host.shadowRoot ?? host.attachShadow({ mode: "open" });
...
function ReactInspector({ report, sharedDateRange = false }: { report: Report; sharedDateRange?: boolean }): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    if (trigger?.isConnected) globalThis.requestAnimationFrame(() => trigger.focus({ preventScroll: true }));
  }, [selectedSession]);
...
function WorkbenchCard({ item, commits, collapsed, onToggle, onOpen }: { item: Item; commits: Commit[]; collapsed: boolean; onToggle(): void; onOpen(session: Session, trigger?: HTMLElement): void }): React.JSX.Element {
  ...
  const [promptWidth, setPromptWidth] = useState(260);
  ...
  const prompt = Math.min(promptWidth, Math.max(180, width - 492));
  const activity = Math.min(activityWidth, Math.max(240, width - prompt - 252));
...
function ProcessingBreakdown({ session, report }: { session: Session; report: UsageReport }): React.JSX.Element | null {
  ...
  return <section className="usage-report-section">
    <header><div><h4>{t("usage.processedBreakdownTitle")}</h4></div><strong>{t("usage.processedLabel", { count: formatTokenCount(total) })}</strong></header>
...
function SessionUsageReport({ session }: { session: Session }): React.JSX.Element {
  ...
  return <section className="session-mode-panel usage-report" aria-label={t("usage.reportAria")}>
    <section className="usage-report-section"><header><h3>{t("usage.contextProgressionTitle")}</h3></header><UsageExplorer session={session} report={report} /></section>
...
const REACT_CSS = `
  ...
  ${LINK_FOCUS_CSS}
  ${DOCKED_PANE_CSS}
.native-inspector-root .workspace-footer{position:static;border-top:1px solid var(--color-border);border-bottom:0;background:var(--color-statusbar,var(--color-surface-subtle));min-height:28px}
.native-inspector-root .workbench-grid{display:grid;min-width:0;grid-template-columns:minmax(180px,var(--prompt-width)) 6px minmax(240px,var(--activity-width)) 6px minmax(240px,1fr)}
.native-inspector-root .workbench{container-type:inline-size}
.native-inspector-root .session-view>.session-nav{position:sticky;top:0;left:0;right:auto;min-height:44px;padding:var(--space-xs) var(--space-md);justify-content:flex-start;gap:var(--space-md);border-bottom:1px solid var(--color-border);background:var(--color-surface)}
.native-inspector-root .session-close{position:static;display:inline-flex;align-items:center;gap:var(--space-xs);flex:none}
.native-inspector-root .session-titlebar{grid-template-columns:minmax(0,1fr) auto;padding-inline:var(--space-md)}
.native-inspector-root .usage-focus-layout{grid-template-columns:minmax(0,1fr)}
.native-inspector-root .usage-focus-layout .usage-context-chart{border:0}
.session-other-records{padding:var(--space-md);min-width:0}.session-other-records summary{cursor:pointer}.session-other-records p{color:var(--color-text-muted)}
@container (max-width:780px){.native-inspector-root .workbench-grid{display:block}.native-inspector-root .workbench-grid>.studio-pane-sash{display:none}}
@media(max-width:760px){.native-inspector-root .session-titlebar{grid-template-columns:minmax(0,1fr);height:auto}.native-inspector-root .workspace-footer .window-badge{display:block}}
