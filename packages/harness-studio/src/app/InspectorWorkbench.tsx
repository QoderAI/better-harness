...
function SessionTrace({ session, commits, generatedAt, onViewUsage }: { session: Session; commits: Commit[]; generatedAt?: string; onViewUsage(): void }): React.JSX.Element {
  ...
  return <section id="react-session-panel-trace" role="tabpanel" aria-labelledby="react-session-tab-trace" className="session-mode-panel"><div className="session-layout"><main className="session-notebook-main"><div className="session-timeline" aria-label={t("session.cellsAria")}>
  ...
  </div></main><aside className="session-sidebar" aria-label={t("session.outlineAria")}><header><div><strong>{t("session.outline")}</strong><span>{t("session.readOnly")}</span></div></header><section><h3>{t("session.cells")}</h3><select className="jump-select" aria-label={t("session.jumpAria")} defaultValue={turns[0]?.turn.anchorId ?? `turn-${turns[0]?.turn.index ?? 1}`} onChange={(event) => jumpTo(event.target.value)}>{turns.map(({ turn }) => <option value={turn.anchorId ?? `turn-${turn.index}`} key={turn.index}>{t("turn.inMarker", { index: turn.index })}</option>)}{(projection.unplacedCalls.length > 0 || projection.unplacedFiles.length > 0) && <option value="unplaced">{t("session.unplaced")}</option>}{projection.outsideCommits.length > 0 && <option value="outside">{t("session.outside")}</option>}</select><div className="session-bulk"><button type="button" onClick={() => setAllProcesses(true)}>{t("session.expandProcess")}</button><button type="button" onClick={() => setAllProcesses(false)}>{t("session.collapseProcess")}</button></div></section><details className="session-filter-disclosure"><summary><span>{t("session.filtersSummary")}</span><em>{t("session.filterCalls", { count: visibleCalls })}</em></summary><div className="session-filter-list"><Filter label={t("session.filterLabels.prompts")} count={turns.length} checked={kinds.has("prompts")} onChange={() => toggleKind("prompts")} /><Filter label={t("session.filterLabels.results")} count={responseCount} checked={kinds.has("responses")} onChange={() => toggleKind("responses")} /><Filter label={t("session.filterLabels.intermediate")} count={noteCount} checked={kinds.has("intermediate")} onChange={() => toggleKind("intermediate")} /><Filter label={t("session.filterLabels.modelUsage")} count={usageCount} checked={kinds.has("usage")} onChange={() => toggleKind("usage")} /><Filter label={t("session.filterLabels.commits")} count={commits.length} checked={kinds.has("commits")} onChange={() => toggleKind("commits")} /><Filter label={t("session.filterLabels.toolCalls")} count={visibleCalls} checked={kinds.has("tools")} onChange={() => toggleKind("tools")} />{toolNames.slice(0, 8).map((name) => <Filter subtype label={name} count={calls.filter((call) => (call.toolName ?? call.operation ?? "tool") === name).length} checked={enabledTools.has(name)} onChange={() => toggleTool(name)} key={name} />)}<Filter subtype label={t("session.filterLabels.filePaths")} count={session.toolActivity?.files?.length ?? session.files?.length ?? 0} checked={showFiles} onChange={() => setShowFiles((value) => !value)} /></div></details><section className="session-outline-facts"><h3>{t("session.facts.session")}</h3><dl><div><dt>{t("session.facts.source")}</dt><dd>{session.source === "entire-checkpoint" ? t("session.facts.entireCheckpoint") : t("session.facts.nativeSession")}</dd></div><div><dt>{t("session.facts.runtime")}</dt><dd>{session.platform ?? t("meta.unknown")}</dd></div><div><dt>{t("session.facts.model")}</dt><dd title={session.models?.join(", ") || t("usage.unavailable")}>{session.models?.join(", ") || t("usage.unavailable")}</dd></div><div><dt>{t("session.facts.duration")}</dt><dd>{formatDuration(session.durationMs)}</dd></div><div><dt>{t("session.facts.turns")}</dt><dd>{turns.length}</dd></div><div><dt>{t("session.facts.toolCalls")}</dt><dd>{calls.length}</dd></div><div><dt>{t("session.facts.fileEdits")}</dt><dd>{session.fileEditCount ?? 0}</dd></div>{session.dialogue?.truncated && <div><dt>{t("session.facts.projection")}</dt><dd>{t("session.facts.truncated")}</dd></div>}</dl></section><UsageContextSummary session={session} generatedAt={generatedAt} onViewReport={onViewUsage} /></aside></div></section>;
}
...
function UsageExplorer({ session, report }: { session: Session; report: UsageReport }): React.JSX.Element {
  ...
  return <div className="usage-linked-explorer">
    <div className="usage-overview"><div className="chart-toolbar"><span className="chart-basis">{t("usage.overviewBasis", { responses: entries.length, prompts: promptEntries.length })}</span><span className="chart-range">{timeRange}</span></div><svg data-usage-overview-chart tabIndex={0} viewBox={`0 0 ${width} 106`} role="img" aria-label={t("usage.overviewChartAria")} onKeyDown={onOverviewKeyDown} onClick={(event) => { const position = positionAt(event.clientX, event.currentTarget.getBoundingClientRect(), false); windowAt(position - Math.floor(size / 2)); select(position, false); }}><rect className="usage-overview-surface" x={padX} y="8" width={width - padX * 2} height="88" /><rect className="usage-overview-brush" x={brushX} y="8" width={Math.max(6, brushEnd - brushX)} height="88" />{usageSegments(entries).filter((segment) => segment.length > 1).map((segment, index) => <path className="usage-chart-line" d={usageStepPath(segment, overviewX, overviewY)} key={`overview-${index}`} />)}{promptEntries.map((entry) => { const markerX = overviewX(entry); const label = Number.isFinite(entry.point.turnIndex) ? `T${entry.point.turnIndex}` : "P"; const chipWidth = Math.max(18, 10 + label.length * 5); const halfWidth = chipWidth / 2; const chipX = Math.max(padX + halfWidth, Math.min(width - padX - halfWidth, markerX)); const tooltipWidth = 250; const tooltipX = Math.max(padX, Math.min(width - padX - tooltipWidth, markerX - tooltipWidth / 2)); const hitX = markerX - 7; const hitRight = markerX + 7; const detail = usagePointDetail(entry.point, t, retainedUsagePrompt(session, entry.point) ?? undefined); return <g className={`usage-overview-turn-marker${entry.position === selected || Number.isFinite(selectedEntry?.point.turnIndex) && entry.point.turnIndex === selectedEntry?.point.turnIndex ? " selected" : ""}`} data-usage-overview-turn-marker data-usage-response-position={entry.position} aria-label={`${detail.primary}. ${detail.secondary}`} onClick={(event) => { event.stopPropagation(); windowAt(entry.position - Math.floor(size / 2)); setSelected(entry.position); }} key={`overview-turn-${entry.point.turnIndex ?? entry.point.id}`}><title>{detail.primary}. {detail.secondary}</title><rect className="usage-overview-turn-hit" x={hitX} y="16" width={hitRight - hitX} height="34" /><line className="usage-overview-turn" x1={markerX} x2={markerX} y1="24" y2="78" /><rect className="usage-overview-turn-chip" x={chipX - halfWidth} y="17" width={chipWidth} height="12" rx="2" /><text className="usage-overview-turn-label" x={chipX} y="26" textAnchor="middle">{label}</text><foreignObject className="usage-overview-prompt-tooltip" x={tooltipX} y="12" width={tooltipWidth} height="38"><div role="tooltip"><strong>{detail.primary}</strong><span>{detail.secondary}</span></div></foreignObject></g>; })}{entries.filter((entry) => ["shrink", "model-change"].includes(entry.point.boundary)).map((entry) => <path className={`usage-overview-event boundary-${entry.point.boundary}`} d={`M${overviewX(entry)} 82 l4 4 -4 4 -4 -4z`} key={`overview-event-${entry.point.id}`} />)}<rect className="usage-overview-handle start" x={brushX - 4} y="52" width="8" height="44" onPointerDown={(event) => beginBrushDrag("start", event)} onPointerMove={moveBrushDrag} onPointerUp={endBrushDrag} onClick={(event) => event.stopPropagation()} /><rect className="usage-overview-handle end" x={brushEnd - 4} y="52" width="8" height="44" onPointerDown={(event) => beginBrushDrag("end", event)} onPointerMove={moveBrushDrag} onPointerUp={endBrushDrag} onClick={(event) => event.stopPropagation()} /><text x={padX} y="14">{formatTokenCount(overviewMax)}</text><text x={padX} y="102">{formatTokenCount(overviewMin)}</text></svg></div>
    <div className="usage-window-toolbar"><div className="usage-window-summary"><strong>{t("usage.responsesRange", { first, last })}</strong><span>{t("usage.responsesCount", { shown: visible.length, total: entries.length })}</span></div><button type="button" disabled={start === 0} onClick={() => windowAt(start - size)}>{t("usage.previousWindow")}</button><div className="usage-window-edge-controls"><label>{t("usage.windowStart")}<input type="range" min="0" max={Math.max(0, end - minSize)} value={start} aria-label={t("usage.windowStartAria")} onChange={(event) => windowEdgeAt("start", Number(event.target.value))} /></label><label>{t("usage.windowEnd")}<input type="range" min={Math.min(entries.length, start + minSize)} max={entries.length} value={end} aria-label={t("usage.windowEndAria")} onChange={(event) => windowEdgeAt("end", Number(event.target.value))} /></label></div><button type="button" disabled={end === entries.length} onClick={() => windowAt(start + size)}>{t("usage.nextWindow")}</button></div>
    <div className="usage-focus-layout"><div className="usage-context-chart"><div className="chart-toolbar"><span className="chart-basis">{t("usage.focusBasis", { first, last })}</span><span className="chart-range">{t("usage.focusHint")}</span></div><svg className="usage-focus-chart" tabIndex={0} viewBox={`0 0 ${width} 180`} role="img" aria-label={t("usage.focusChartAria", { first, last })} onKeyDown={onExplorerKeyDown} onClick={(event) => select(positionAt(event.clientX, event.currentTarget.getBoundingClientRect(), true))}><rect className="usage-focus-surface" x={padX} y="8" width={width - padX * 2} height="158" />{[0, 0.5, 1].map((ratio) => { const lineY = focusTop + ratio * (focusBottom - focusTop); return <line className="usage-chart-grid" x1={padX} x2={width - padX} y1={lineY} y2={lineY} key={ratio} />; })}{usageSegments(visible).filter((segment) => segment.length > 1).map((segment, index) => <path className="usage-chart-line" d={usageStepPath(segment, focusX, focusY)} key={`focus-${index}`} />)}{selectedVisible && <><line className="usage-selection-line" x1={focusX(selectedEntry)} x2={focusX(selectedEntry)} y1={focusTop} y2="164" /><circle className="usage-selection-point" cx={focusX(selectedEntry)} cy={focusY(selectedEntry)} r="5" /></>}{visible.filter((entry) => ["shrink", "model-change"].includes(entry.point.boundary)).map((entry) => { const markerX = focusX(entry); const detail = usagePointDetail(entry.point, t, retainedUsagePrompt(session, entry.point) ?? undefined); return <g aria-label={`${detail.primary}. ${detail.secondary}`} onClick={(event) => { event.stopPropagation(); select(entry.position); }} key={`focus-event-${entry.point.id}`}><rect className="usage-chart-point-hit" x={markerX - 8} y="140" width="16" height="22" />{entry.point.boundary === "shrink" ? <path className="usage-focus-event boundary-shrink" d={`M${markerX} 147 l4 4 -4 4 -4 -4z`} /> : <rect className="usage-focus-event boundary-model-change" x={markerX - 4} y="147" width="8" height="8" />}</g>; })}<text x={padX} y={focusTop - 4}>{formatTokenCount(focusMax)}</text><text x={padX} y="172">{formatTokenCount(focusMin)}</text></svg><div className="usage-chart-legend"><span><i className="growth" />{t("usage.chartLegendSnapshot")}</span><span><i className="shrink" />{t("usage.chartLegendShrink")}</span><span><i className="boundary" />{t("usage.chartLegendBoundary")}</span></div></div><ResponseDetails point={selectedEntry?.point} prompt={selectedEntry ? retainedUsagePrompt(session, selectedEntry.point) : null} onStep={move} t={t} /></div>
    <div className="usage-response-table" role="listbox" aria-label={t("usage.responseTableAria", { first, last })} onKeyDown={onExplorerKeyDown} ref={tableRef}><div className={`usage-response-head${processedVisible ? " with-processed" : ""}`} aria-hidden="true"><span>{t("usage.progressResponse")}</span><span>{t("usage.progressTime")}</span><span className="numeric-cell">{t("usage.progressContext")}</span><span className="numeric-cell">{t("usage.progressDelta")}</span><span>{t("usage.progressReuse")}</span>{processedVisible && <span className="numeric-cell">{t("usage.progressProcessed")}</span>}<span className="numeric-cell">{t("usage.progressOutput")}</span></div><ol>{visible.map((entry) => { const point = entry.point; const isSelected = entry.position === selected; return <li className={`usage-response-row${processedVisible ? " with-processed" : ""}${isSelected ? " selected" : ""}`} role="option" aria-selected={isSelected} tabIndex={isSelected || selected < 0 && entry === visible[0] ? 0 : -1} onClick={() => select(entry.position)} ref={isSelected ? selectedRowRef : undefined} key={point.id}><strong>{t("usage.responseIndex", { index: point.index })}</strong><span>{formatUsageStamp(point.timestamp) ?? "—"}</span><strong className="numeric-cell">{Number.isFinite(point.contextTokens) ? formatTokenCount(Number(point.contextTokens)) : "—"}</strong><span className="numeric-cell usage-delta">{Number.isFinite(point.contextDeltaTokens) ? formatSignedTokenCount(Number(point.contextDeltaTokens)) : "—"}</span><em>{compactReuse(point.cacheReuse, t)}</em>{processedVisible && <span className="numeric-cell">{Number.isFinite(point.processedTokens) ? formatTokenCount(Number(point.processedTokens)) : "—"}</span>}<span className="numeric-cell">{Number.isFinite(point.outputTokens) ? formatTokenCount(Number(point.outputTokens)) : "—"}</span></li>; })}</ol></div>
  </div>;
}
...
function UsageContextSummary({ session, generatedAt, onViewReport }: { session: Session; generatedAt?: string; onViewReport(): void }): React.JSX.Element {
  ...
  if (Number.isFinite(report.processedTokens)) metrics.push({ label: t("usage.sessionProcessed"), value: formatTokenCount(Number(report.processedTokens)) });
  else if (Number.isFinite(report.providerTotalTokens)) metrics.push({ label: t("usage.providerTotal"), value: formatTokenCount(Number(report.providerTotalTokens)) });
...
function UsageEvidenceDetails({ session, report, freshness }: { session: Session; report: UsageReport; freshness: ReturnType<typeof usageSnapshotFreshness> }): React.JSX.Element {
  ...
  const groups: Array<{ label: string; facts: Array<[string, string | number]> }> = [
    { label: "Observability", facts: [["Coverage", coverage], ["Snapshot", freshness.evidence], ["Time basis", session.timestampBasis ?? t("usage.unobserved")], ["Raw context", "omitted"]] },
  ...
  return <section className="usage-report-evidence" aria-label={t("usage.evidenceDetailsTitle")}>
...
function UsageReportOccupancyTile({ session, report, context }: { session: Session; report: UsageReport; context: ReturnType<typeof usageContextPresentation> }): React.JSX.Element {
  ...
  const boundaryTitle = compactionCount > 0
    ? t("usage.compactionBoundaries", { count: compactionCount })
  ...
  const label = context.hasUsedTokens ? t("usage.latestContext") : context.hasPercentFull ? t("usage.latestOccupancy") : t("usage.occupancyUnavailable");
  const detail = context.hasContextWindow ? t("usage.windowSummary", { used: formatTokenCount(context.usedTokens), window: formatTokenCount(context.windowTokens), percent: context.percentFull, net: "" })
    : context.hasPercentFull ? t("usage.windowSizeNotObserved") : context.hasUsedTokens ? t("usage.contextWindowNotObserved") : t("usage.noContextEvidence");
  return <div className="usage-report-occupancy"><strong>{value}</strong><div className="usage-summary-tile-heading"><span>{label}</span>{boundaryLabel && <em className="usage-summary-compactions" title={boundaryTitle}>{boundaryLabel}</em>}</div><small>{detail}</small>{context.hasContextWindow ? <ContextUsageBar segments={context.segments} unusedTokens={context.unusedTokens} label={t("usage.windowFullAria", { count: context.percentFull })} t={t} /> : context.hasPercentFull ? <ContextOccupancyBar percentFull={context.percentFull} label={t("usage.occupancyAria", { count: context.percentFull })} /> : null}</div>;
}
...
function ContextStructure({ session }: { session: Session }): React.JSX.Element {
  ...
  return <section className="usage-report-section usage-structure-section">
    ...
    {layers.length > 0 ? <>
      <div className="usage-structure-bar" role="img" aria-label={t("usage.structureAria", { total, count: layers.length })}>
        {layers.map((layer, index) => <i className={`category-${index % 8}`} style={{ flexGrow: layer.itemCount }} title={t("usage.layerItemCount", { kind: layer.kind, count: layer.itemCount })} key={`${layer.kind}-${index}`} />)}
      </div>
...
function SessionUsageReport({ session, generatedAt }: { session: Session; generatedAt?: string }): React.JSX.Element {
  ...
  const processedOrPeak = Number.isFinite(report.processedTokens)
    ? { label: t("usage.sessionProcessed"), value: formatTokenCount(Number(report.processedTokens)) }
  ...
  return <section className="session-mode-panel usage-report" aria-label={t("usage.reportAria")}>
    <header className="usage-report-lead"><h3 className="visually-hidden">{t("usage.reportTitle")}</h3><UsageEvidenceDetails session={session} report={report} freshness={freshness} /><aside className="usage-report-summary" aria-label={t("usage.sessionUsageSummaryAria")}><UsageReportOccupancyTile session={session} report={report} context={context} /><dl className="usage-report-lead-facts"><UsageReportReuseTile reuse={cacheReuse} /><div><dt>{t("usage.baselineContext")}</dt><dd>{Number.isFinite(report.baselineContextTokens) ? formatTokenCount(Number(report.baselineContextTokens)) : t("usage.notObserved")}</dd></div><div><dt>{t("usage.netVsBaseline")}</dt><dd>{Number.isFinite(report.netContextDeltaTokens) ? formatSignedTokenCount(Number(report.netContextDeltaTokens)) : t("usage.notComparable")}</dd></div><div><dt>{processedOrPeak.label}</dt><dd>{processedOrPeak.value}</dd></div><div><dt>{t("usage.modelCalls")}</dt><dd>{report.actualModelCalls || t("usage.notObserved")}</dd></div></dl></aside></header>
