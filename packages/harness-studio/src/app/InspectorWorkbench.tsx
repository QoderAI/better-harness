...
function SessionTrace({ session, commits, generatedAt, onViewUsage }: { session: Session; commits: Commit[]; generatedAt?: string; onViewUsage(): void }): React.JSX.Element {
  ...
  const primaryModel = session.models?.[0] ?? t("usage.unavailable");
  const modelRemainder = (session.models?.length ?? 0) > 1 ? ` +${(session.models?.length ?? 1) - 1}` : "";
  const sessionFactSummary = `${session.platform ?? t("meta.unknown")} · ${primaryModel}${modelRemainder}`;
  ...
  </div></main><aside className="session-sidebar" aria-label={t("session.outlineAria")}><header><strong>{t("session.outline")}</strong><span>{t("session.readOnly")}</span></header><section className="session-outline-controls"><select className="jump-select" aria-label={t("session.jumpAria")} defaultValue={turns[0]?.turn.anchorId ?? `turn-${turns[0]?.turn.index ?? 1}`} onChange={(event) => jumpTo(event.target.value)}>{turns.map(({ turn }) => <option value={turn.anchorId ?? `turn-${turn.index}`} key={turn.index}>{t("turn.inMarker", { index: turn.index })}</option>)}{(projection.unplacedCalls.length > 0 || projection.unplacedFiles.length > 0) && <option value="unplaced">{t("session.unplaced")}</option>}{projection.outsideCommits.length > 0 && <option value="outside">{t("session.outside")}</option>}</select><div className="session-bulk"><button type="button" aria-label={t("session.expandProcess")} onClick={() => setAllProcesses(true)}>{t("session.expandShort")}</button><button type="button" aria-label={t("session.collapseProcess")} onClick={() => setAllProcesses(false)}>{t("session.collapseShort")}</button></div></section><details className="session-filter-disclosure"><summary><span>{t("session.filtersSummary")}</span><em>{t("session.filterCalls", { count: visibleCalls })}</em></summary><div className="session-filter-list"><Filter label={t("session.filterLabels.prompts")} count={turns.length} checked={kinds.has("prompts")} onChange={() => toggleKind("prompts")} /><Filter label={t("session.filterLabels.results")} count={responseCount} checked={kinds.has("responses")} onChange={() => toggleKind("responses")} /><Filter label={t("session.filterLabels.intermediate")} count={noteCount} checked={kinds.has("intermediate")} onChange={() => toggleKind("intermediate")} /><Filter label={t("session.filterLabels.modelUsage")} count={usageCount} checked={kinds.has("usage")} onChange={() => toggleKind("usage")} /><Filter label={t("session.filterLabels.commits")} count={commits.length} checked={kinds.has("commits")} onChange={() => toggleKind("commits")} /><Filter label={t("session.filterLabels.toolCalls")} count={visibleCalls} checked={kinds.has("tools")} onChange={() => toggleKind("tools")} />{toolNames.slice(0, 8).map((name) => <Filter subtype label={name} count={calls.filter((call) => (call.toolName ?? call.operation ?? "tool") === name).length} checked={enabledTools.has(name)} onChange={() => toggleTool(name)} key={name} />)}<Filter subtype label={t("session.filterLabels.filePaths")} count={session.toolActivity?.files?.length ?? session.files?.length ?? 0} checked={showFiles} onChange={() => setShowFiles((value) => !value)} /></div></details><details className="session-facts-disclosure"><summary><span>{t("session.facts.session")}</span><em title={`${session.platform ?? t("meta.unknown")} · ${session.models?.join(", ") || t("usage.unavailable")}`}>{sessionFactSummary}</em></summary><section className="session-outline-facts"><h3 className="visually-hidden">{t("session.facts.session")}</h3><dl><div><dt>{t("session.facts.source")}</dt><dd>{session.source === "entire-checkpoint" ? t("session.facts.entireCheckpoint") : t("session.facts.nativeSession")}</dd></div><div><dt>{t("session.facts.runtime")}</dt><dd>{session.platform ?? t("meta.unknown")}</dd></div><div><dt>{t("session.facts.model")}</dt><dd title={session.models?.join(", ") || t("usage.unavailable")}>{session.models?.join(", ") || t("usage.unavailable")}</dd></div><div><dt>{t("session.facts.duration")}</dt><dd>{formatDuration(session.durationMs)}</dd></div><div><dt>{t("session.facts.turns")}</dt><dd>{turns.length}</dd></div><div><dt>{t("session.facts.toolCalls")}</dt><dd>{calls.length}</dd></div><div><dt>{t("session.facts.fileEdits")}</dt><dd>{session.fileEditCount ?? 0}</dd></div>{session.dialogue?.truncated && <div><dt>{t("session.facts.projection")}</dt><dd>{t("session.facts.truncated")}</dd></div>}</dl></section></details><UsageContextSummary session={session} generatedAt={generatedAt} onViewReport={onViewUsage} /></aside></div></section>;
}
