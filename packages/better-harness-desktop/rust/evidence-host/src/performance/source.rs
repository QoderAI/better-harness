...
use super::{reader::safe_directory, SourceRequest};
...
/// `<index>/<bucket>/<file>.jsonl` — a portable evidence identifier, never a
/// filesystem path supplied by the client. Only one of the session's own
/// discovered locations is ever resolved.
fn parts(request: &SourceRequest, bucket: &str) -> Result<(usize, String), String> {
    let parts: Vec<_> = request.source.split('/').collect();
    if parts.len() != 3
        || parts[1] != bucket
        || !parts[2].ends_with(".jsonl")
        || parts[2].contains('\\')
        || parts[2].contains(':')
        || request.line == 0
    ...
    Ok((index, parts[2].to_string()))
}

/// Qoder keeps one directory of rotated segments per Session.
pub fn read_segments(
    home: &Path,
    dirs: &[PathBuf],
    request: &SourceRequest,
) -> Result<Value, String> {
    let (index, name) = parts(request, "segments")?;
    let dir = dirs.get(index).ok_or("source-not-found")?.join("segments");
    if !safe_directory(&dir, home) {
        return Err("source-not-found".into());
    }
    open_window(&dir.join(name), request)
}

/// The native formats keep one transcript file per Session. The reference must
/// name the file the reader already discovered, so a crafted reference cannot
/// reach a sibling transcript.
pub fn read_files(
    home: &Path,
    files: &[PathBuf],
    request: &SourceRequest,
) -> Result<Value, String> {
    let (index, name) = parts(request, "transcript")?;
    let path = files.get(index).ok_or("source-not-found")?;
    if path
        .file_name()
    ...
    open_window(path, request)
}

fn open_window(path: &Path, request: &SourceRequest) -> Result<Value, String> {
...
fn window<R: BufRead>(mut reader: R, request: &SourceRequest) -> Result<Value, String> {
    ...
    for number in 1..=end {
        ...
        loop {
            ...
            if number >= start && length < LINE_BYTES {
                bytes.extend_from_slice(&buffer[..size.min(LINE_BYTES - length)]);
            }
            length += size;
            scanned += size as u64;
            reader.consume(size);
        ...
        if number >= start {
            ...
            let text = if length > LINE_BYTES || (!terminated && scanned >= SCAN_BYTES) {
                ...
                let safe =
                    crate::privacy::redact_private_text(String::from_utf8_lossy(&bytes).trim_end());
    ...
    if !found {
        return Err(if scanned >= SCAN_BYTES {
            "source-scan-limit"
        } else {
    ...
    Ok(
        json!({"schemaVersion":1,"engine":"rust","source":request.source,"line":request.line,
...
mod tests {
    ...
    fn scans_only_to_context_and_preserves_line_numbers() {
        ...
        let result = window(
            Cursor::new(&input),
            &SourceRequest {
                source: "1/segments/a.jsonl".into(),
                line: 44,
            },
        )
        .unwrap();
    ...
    fn bounds_long_lines_redacts_secrets_and_reports_missing_target() {
        ...
        assert_eq!(
            call_summary(&json!({"args":{"command":"git status"}})).unwrap(),
            "git status"
        );
    }
}
