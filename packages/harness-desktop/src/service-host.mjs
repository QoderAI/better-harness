...
export function connectStudioService(child, { token, dataDirectory, oxcExecutable, pickDirectory, onFailure, startupTimeout = 30_000, shutdownTimeout = 5_000 }) {
  let ready = false;
  let stopping = false;
  let exited = false;
  let picking = false;
  ...
  child.postMessage(message('start', { token, dataDirectory, oxcExecutable }));
  return {
    started,
    stop() {
