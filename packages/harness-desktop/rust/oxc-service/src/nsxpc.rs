//! Foundation NSXPC transport. The JSONL bridge contains no compiler calls.
...
const SERVICE: &str = "com.qoder.harness-studio.oxc";
// Bound total compiler concurrency and memory across connections.
static COMPILER: Mutex<()> = Mutex::new(());
unsafe extern "C" {
    fn harness_oxc_protocol() -> *const AnyProtocol;
}
fn interface() -> Retained<NSXPCInterface> {
    ...
    unsafe { NSXPCInterface::interfaceWithProtocol(&*harness_oxc_protocol()) }
}

define_class!(
    #[unsafe(super = NSObject)]
    struct OxcService;
    unsafe impl NSObjectProtocol for OxcService {}
    impl OxcService {
        ...
        fn perform_request(&self, request: &NSData, reply: &DynBlock<dyn Fn(NonNull<NSData>)>) {
            autoreleasepool(|_| {
                let result = std::panic::catch_unwind(|| {
                    ...
                    let _lock = COMPILER.lock().unwrap_or_else(|poison| poison.into_inner());
                    // A disconnected client cannot cancel synchronous OXC. Bound
                    ...
                    let result = crate::handle(&request.to_vec());
                    drop(done);
                    let _ = watchdog.join();
                    result
                });
                let value = result.unwrap_or_else(|_| serde_json::json!({"version":1,"id":null,"error":{"code":"compiler-panic"}}));
...
define_class!(
    ...
    unsafe impl NSXPCListenerDelegate for ListenerDelegate {
        ...
        fn accept(&self, _listener: &NSXPCListener, connection: &NSXPCConnection) -> bool {
            ...
            let object: Retained<OxcService> = unsafe { msg_send![OxcService::alloc(), init] };
            unsafe {
...
/// One connection per bridge. No source enters the client compiler core.
